...
public sealed class SdafTlv
{
    ...
    public static SdafTlv UInt16(ushort tag, ushort value) { byte[] b = new byte[2]; Bin.U16(b, value); return new(tag, SdafWireType.UInt16, b); }
    public static SdafTlv UInt32(ushort tag, uint value) { byte[] b = new byte[4]; Bin.U32(b, value); return new(tag, SdafWireType.UInt32, b); }
    public static SdafTlv UInt64(ushort tag, ulong value) { byte[] b = new byte[8]; Bin.U64(b, value); return new(tag, SdafWireType.UInt64, b); }
    public static SdafTlv Int64(ushort tag, long value) { byte[] b = new byte[8]; Bin.I64(b, value); return new(tag, SdafWireType.Int64, b); }
    public static SdafTlv Float64(ushort tag, double value) => Int64(tag, BitConverter.DoubleToInt64Bits(value)).WithWireType(SdafWireType.Float64);
    public static SdafTlv Utf8(ushort tag, string value) => new(tag, SdafWireType.Utf8, Bin.Utf8.GetBytes(value));
    public static SdafTlv Bytes(ushort tag, byte[] value) => new(tag, SdafWireType.Bytes, value);
    public static SdafTlv Boolean(ushort tag, bool value) => new(tag, SdafWireType.Boolean, [value ? (byte)1 : (byte)0]);
    public static SdafTlv Rational(ushort tag, ulong numerator, ulong denominator)
    {
        ...
        byte[] b = new byte[16]; Bin.U64(b, numerator); Bin.U64(b.AsSpan(8), denominator);
        return new(tag, SdafWireType.RationalUInt64, b);
    }
    public static SdafTlv Rational(ushort tag, long numerator, ulong denominator)
    {
        if (denominator == 0) throw new ArgumentOutOfRangeException(nameof(denominator));
        byte[] b = new byte[16]; Bin.I64(b, numerator); Bin.U64(b.AsSpan(8), denominator);
    ...
    public (ulong Numerator, ulong Denominator) AsRationalUInt64() => (Bin.U64(Value), Bin.U64(Value.AsSpan(8)));
    public (long Numerator, ulong Denominator) AsRationalInt64() => (Bin.I64(Value), Bin.U64(Value.AsSpan(8)));
    ...
    internal static void ValidateWireSize(SdafWireType type, int size)
    {
        ...
        if (expected >= 0 && size != expected) throw new SdafFormatException($"Wire type {type} requires {expected} bytes, not {size}.");
    }
}
...
public sealed class SdafSchema
{
    ...
    {
        if (id == 0 || revision == 0) throw new ArgumentOutOfRangeException(id == 0 ? nameof(id) : nameof(revision));
        Id = id;
        Revision = revision;
        Objects = objects.ToArray();
    }

    public SdafSchemaObject? Find(SdafObjectKind kind, uint id) => Objects.FirstOrDefault(x => x.Kind == kind && x.Id == id);
    public IReadOnlyList<SdafSchemaObject> Channels(uint streamId) => Objects
