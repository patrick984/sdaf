...
internal static partial class Zstandard
{
    ...
    private static unsafe partial nuint CompressNative(byte* dst, nuint dstCapacity, byte* src, nuint srcSize, int level);
    [LibraryImport(Library, EntryPoint = "ZSTD_decompress")]
    private static unsafe partial nuint DecompressNative(byte* dst, nuint dstCapacity, byte* src, nuint compressedSize);
    ...
    internal static unsafe byte[] Compress(ReadOnlySpan<byte> source, int level = 3)
    {
        ...
        {
            nuint size = CompressNative(dst, (nuint)destination.Length, src, (nuint)source.Length, level);
            Check(size);
    ...
    internal static unsafe byte[] Decompress(ReadOnlySpan<byte> source, int expectedSize)
    {
        ...
        fixed (byte* dst = destination)
        {
            nuint size = DecompressNative(dst, (nuint)destination.Length, src, (nuint)source.Length);
            Check(size);
            if (size != (nuint)expectedSize) throw new SdafFormatException($"Zstandard output was {size} bytes; expected {expectedSize}.");
        }
        return destination;
    }
